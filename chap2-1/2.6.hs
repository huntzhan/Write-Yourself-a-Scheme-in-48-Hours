...
-- Wrong way to chain.
-- parseFloat = do float <- try (many digit >> char '.' >> many digit)
--                 return . Float . fst . head . readFloat $ float
parseFloat = do real_part <- many digit
                char '.'
                frac_part <- many digit
                return . Float . fst . head . readFloat $ real_part ++ ['.'] ++ frac_part
